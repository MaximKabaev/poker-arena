...
export function ActionPanel({ allowed, bigBlind, potChips, submitting, onSubmit }: Props) {
  ...
  // Initialize amount sliders with sensible defaults.
  ...
  useEffect(() => {
    ...
    if (allowed.canRaise) {
      // pot-sized raise default ≈ callToAmount + pot
  ...
  return (
    <div className="bg-zinc-900/95 backdrop-blur border border-zinc-800 rounded-xl p-4 space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3 text-xs text-zinc-400">
        ...
        <div>
          Min raise to: <span className="font-mono text-zinc-200">{allowed.minRaiseTo ?? "—"}</span>
        </div>
        <div>
      ...
      <div>
        <label className="block text-xs text-zinc-400 mb-1">Chat message (required)</label>
        <input
          type="text"
          maxLength={500}
          value={message}
      ...
      <div>
        <label className="block text-xs text-zinc-400 mb-1">Reasoning (optional, ≤150 chars)</label>
        <input
          type="text"
          maxLength={150}
          value={reasoning}
          onChange={(e) => setReasoning(e.target.value)}
          placeholder='e.g. {vr: "JJ+/AK", ke: "GTO 60%", pp: "IP barrel", sr: "1/2 pot"}'
          className="w-full bg-zinc-800 border border-zinc-700 rounded-md px-3 py-2 text-xs font-mono outline-none focus:border-blue-500"
        />
      </div>
...
function ActionButton({
  ...
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className={`${color} disabled:opacity-30 disabled:cursor-not-allowed rounded-md py-2 px-3 font-semibold text-sm transition`}
    >
      {label}
    </button>
  );
...
function AmountRow({
  ...
  const presets = useMemo(() => {
    ...
    if (potChips > 0) {
      out.push({ label: "⅓ pot", v: Math.round(potChips * 0.33) });
      out.push({ label: "½ pot", v: Math.round(potChips * 0.5) });
      out.push({ label: "⅔ pot", v: Math.round(potChips * 0.66) });
      ...
      <div className="flex gap-1.5">
        {presets.map((p) => (
          <button
            ...
            className="text-[11px] px-2 py-0.5 rounded bg-zinc-800 hover:bg-zinc-700 border border-zinc-700"
          >
            {p.label}
          </button>
        ))}
        <input
          ...
          className="ml-auto w-28 bg-zinc-800 border border-zinc-700 rounded px-2 py-0.5 text-xs font-mono"
        />
      </div>
      <button
        onClick={onFire}
        disabled={disabled || value < min || value > max}
        className={`w-full ${color} disabled:opacity-30 disabled:cursor-not-allowed rounded-md py-2 font-semibold text-sm transition`}
      >
